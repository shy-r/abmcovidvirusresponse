...
to createpeople
  create-turtles percent_retired_65+ [
    ...
    set leisurelocation houselocation ;set leisure to house for now
  ...
  if (Close_school = true) [ ;if schools are closed, then teachers and students stay at home and engage in e-learning
    ask turtles with [title = "2) teacher" or title = "3) school student"] [
  ...
  if (Close_bar = true) [ ;if bars are closed, then bar and restaurant workers don't get paid and have a freeday
    ask turtles with [title = "2) bar/restaurantworker"] [
      set joblocation houselocation
    ]
  ]
...
to householdmealsrent
  if (ticks > 0 and ticks mod 1440 = 0) [
    ask patches with [plabel = "H"] [
      ...
      set savings savings + count turtles with [title = "1) retired" and houselocation = myself] * daily_pension_retiree ; retirees get their pension payments every day
    ]
  ]
end
...
642
47
MODEL OF HEALTH AND ECONOMIC IMPACTS OF VIRUS RESPONSE
20
0.0
1
...
@#$#@#$#@
<experiments>
  <experiment name="NoResponse_50runs" repetitions="50" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="13000"/>
  ...
  <experiment name="CloseSchool_50runs" repetitions="50" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="13000"/>
    <metric>sum [savings] of patches with [plabel = "H"]</metric>
  ...
  <experiment name="CloseBar_50runs" repetitions="50" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="13000"/>
    <metric>sum [savings] of patches with [plabel = "H"]</metric>
  ...
  <experiment name="Lockdown_50runs" repetitions="50" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="13000"/>
    <metric>sum [savings] of patches with [plabel = "H"]</metric>
  ...
  <experiment name="TestIsolate_50runs" repetitions="50" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="13000"/>
    <metric>sum [savings] of patches with [plabel = "H"]</metric>
